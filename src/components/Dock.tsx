        ...
        <AnimatePresence>
            {isVisible && (
                <motion.div
                    ...
                    className={`${className} absolute -top-6 left-1/2 w-fit whitespace-pre rounded-lg border border-neutral-700 bg-[#060010] px-2 py-0.1 mb-5 text-md text-white`}
                    role="tooltip"
                    style={{ x: '-50%' }}
                >
                    {children}
    ...
    return (
        <motion.div style={{ height, scrollbarWidth: 'none' }} className="mx-2 flex max-w-full items-center">
            <motion.div
                ...
                className={`${className} absolute bottom-2 left-1/2 transform -translate-x-1/2 flex items-center w-fit gap-4 rounded-2xl border-neutral-700 border-2 py-10 px-4`}
